...
- Server running from staging @3cb9468; no frontend rebuild before the
  ...
  - **Correction (state of this machine):** the rehearsal server was
    stopped during Part A cleanup, before the freeze was declared. Start
    it once by hand before the demo (`DEMO_SCRIPT.md` §1:
    `.venv\Scripts\python.exe -m ui.backend.main`) — the built frontend
    at `ui/frontend/dist` is the frozen 3cb9468 build; no rebuild needed
    or wanted.
