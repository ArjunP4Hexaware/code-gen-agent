...
20. **SRC_FILE_DELIMITER for a file that is not delimited.** The walkthrough (§7) says the column holds "sometimes the tab, sometimes the pipe … sometimes fixed width" — i.e. a WORD describing the layout when there is no separator. Every reference IIG leaves the cell BLANK for the fixed-width feed, and the agent follows the goldens (blank + flagged). Which is right: blank, or the word (`fixed width`, `xlsx`)?
21. **SRC_FORMAT vocabulary.** The column is documented only as "file format". The agent writes the file pattern's extension (`csv`, `psv`, `xlsx`). Is there a fixed vocabulary, and what is the spelling for an Excel source?
22. **Where does an inbound spreadsheet's SHEET NAME go?** No framework table has a column for it (no `SRC_SHEET_NAME`, `SHEET` or `TAB` in any IIG layout). The agent keeps it in the contract and the generated reader only. Should it be carried in the metadata DB, and in which column?
23. Are the natural keys the probe looks rows up by correct — `PIPELINE_NAME` (§2), (`GROUP_ID`, `OBJECT_ID`) for the file → ADLS table, (`GROUP_ID`, `OBJECT_ID`, `OBJECT_NAME`) for the two Delta tables (§5, §7), and the by-analogy keys in `config.yaml dml.natural_keys` for the §8 tables?
