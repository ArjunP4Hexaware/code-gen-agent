...
> Every failure below is addressed in v0.5.0-acfc (M8) — see `docs/ACFC_DEPLOY.md` §1 for the fact → consequence table. The config changes it lists (workspace catalog / volumes, the ten-pair `pairing_map`) were never merged.
...
`databricks.catalog` from `soham_workspace` to `<catalog>`,
...
ambiguous (pairs 1/3/4/6 share ticket `<ticket>`).
...
**Run:** Loading config with `default_catalog: <stage catalog>` added under
`conventions:`. 
...
**Error:** `FAIL extract-sttm — sheet '<real sheet name>': the stage band has no values for ['table', 'column', 'target_type']` 
...
`<fingerprint>`. A live model call is needed. 
...
exist_ok=True)` with `pkg_dest = Path("/Volumes/<catalog>/codegen/codegen_outputs/demo/pair_1")`. 
...
auto-selected. 
**Root cause:** Pairs 1/3/4/6 share ticket `<ticket>`; pair 2 has no
...
`/Volumes/<catalog>/codegen/codegen_outputs/state/layout_profiles`. 
...
**Run:** `GRANT USE CATALOG ON CATALOG <catalog> TO \`<app-sp-client-id>\``
**Error:** `PERMISSION_DENIED: User does not have MANAGE on Catalog '<catalog>'.` 
