...
| `sttm/pair_11_family_b.xlsx` | STTM B — FILE_DETAILS + MAPPING- sheets | 11 | the one-block / many-files FRD's STTM: FILE_DETAILS (5 cols, three files with `Yearly Twice` / `Monthly` frequencies — the FRD's Frequency cell points here), three `MAPPING-` sheets whose stage tables the FRD never names, two schemas across the three sheets, and one `_PCT` column typed `String` among `Decimal` siblings (the sibling-type check's target) |
| `frd/f1_pair_11_multi_file.docx` | FRD F1 — one metadata block for several files | 11 | section titles suffixed with requirement IDs; `Object Name` = 5-column NESTED table (one row per file); `ADLS Location` = 2-column nested table (object \| path); `Domain and Subdomain` = per-file blocks under `<File> file Ingestion from <Src>:` headings with `Domain = …` / `Subdomain = …` lines; `Data Source` = `Vendor Files = <description>` (label-prefixed, not the field); `Frequency` = the pointer sentence; `Target Table Name` lists the three FILE names; `Target Schema` two lines, no catalog; Vendor Metadata carries the vendor name. Three feeds derive |
| `frd/f1_pair_11_multi_file_catalog.docx` | FRD F1 — same, catalog stated | 11 | identical except `Target Schema` = `STG: cat_syn.stg_dom_a; STD: cat_syn.dom_a` |
